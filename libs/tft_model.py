...
class TemporalFusionTransformer(object):
  ...
  def fit(self, train_df=None, valid_df=None):
    ...
    tmp_checkpont = self.get_keras_saved_path(self._temp_folder) + '.index'
  ...
  def predict(self, df, return_targets=False):
    ...
    def format_outputs(prediction, quantile):
    ...
    return {k: format_outputs(process_map[k], k) for k in process_map}
